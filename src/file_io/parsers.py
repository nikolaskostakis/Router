...
from structures.design_components import Design, Row, IOPort, Component, Net
...
def practical_format_parcer(file:TextIOWrapper) -> (Design | None):
    ...
    def _core_info_section(file:TextIOWrapper) -> bool:
        ...
        try:
            ...
            parsersLogger.exception("Utilisation is supposed to be: ##%%")
            return False
        except:
        ...
        try:
            ...
            parsersLogger.exception(
                "Width, Height & Aspect Ratio are supposed " \
        ...
        try:
            ...
            parsersLogger.exception(
                "Offsets are supposed to be floating-point numbers")
            return False
        except:
            parsersLogger.exception(
                "File does not follow practical format syntax")
    ...
    def _rows_section(file:TextIOWrapper) -> bool:
        ...
        while True:
            ...
            try:
                ...
                parsersLogger.exception(
                    "Row coordinates are supposed to be floating-point numbers")
                return False
            except:
                parsersLogger.exception(
                    "File does not follow practical format syntax")
    ...
    def io_ports_section(file:TextIOWrapper) -> bool:
        ...
        while True:
            ...
            try:
                ...
                parsersLogger.exception(
                    "IO Port coordinates are supposed to be " \
    ...
    for state in fileSections:
        match state:
            case "Design Info":
                parsersLogger.debug("Parsing Design Information")
                design = _design_info_section(file)
                if not design:
                    return None
            case "Core Info":
                parsersLogger.debug("Parsing Core Information")
                if not _core_info_section(file):
                    break
            case "Rows":
                parsersLogger.debug("Parsing Rows")
                if not _rows_section(file):
                    break
            case "IO Ports":
                parsersLogger.debug("Parsing IO Ports")
                if not io_ports_section(file):
                    break
            case "IO Ports CCs":
                parsersLogger.debug("Parsing IO CCs")
                if not io_ports_ccs_section(file):
                    break
            case "Components":
                parsersLogger.debug("Parsing Components")
                if not components_section(file):
                    break
            case "End":
                parsersLogger.debug("Parsing completed suceessfully")
                # Parsing was successful
                return design
    
    # Parsing was not successful
    parsersLogger.debug("Parsing was not successfull")
    del design
    return None
# End of function
...
def components_location_parser(file:TextIOWrapper, design: Design):
    ...
    def extract_file_components() -> bool:
        ...
        if (design.name != splits[2]):
            parsersLogger.error(
                "Stored file desing is not the same as the design loaded")
            return False
        ...
        while True:
            ...
            try:
                ...
                parsersLogger.exception("Component coordinates are " \
                    "supposed to be floating-point numbers")
                return False
            except:
                parsersLogger.exception("Something wrong has happened")
                return False
            
            # Check if component exists in the design
            if not design.core.get_component(name):
                parsersLogger.error(
                    f"Component {name} does not exist in the design")
                return False
    ...
    if (len(fileComponents) != design.core.noof_components()):
        # Failure
        parsersLogger.error(
            "Inconsistent number of components between file and design")
        return False
